...
class EmployeeAddModel {
    ...
    Role parseRole(String choice) {
        ...
        if (c.equals("1") || c.equalsIgnoreCase("Manager"))
            return Role.MANAGER;
        if (c.equals("2") || c.equalsIgnoreCase("Employee"))
            return Role.EMPLOYEE;
        return null;
    }

    void registerEmployee(String name, String email, String password,
            String mobile, Long dob, Role role, String address,Double salary) {
        ...
        if (saved == null) {
            employeeAddView.showErrorMessage("Could not create account. Please try again.");
            return;
        }
    ...
    public Double calculateSalary(Employee employee) {
        Long joinedMillis = employee.getJoinedDate();
        if (joinedMillis == null) {
            employeeAddView.showErrorMessage("join Date is null");
            return null;
        }

        Calendar joinedCalenderDate = Calendar.getInstance();
        joinedCalenderDate.setTimeInMillis(joinedMillis);

        Calendar presentCalenderDate = Calendar.getInstance();
        int years = presentCalenderDate.get(Calendar.YEAR) - joinedCalenderDate.get(Calendar.YEAR);

        if (presentCalenderDate.get(Calendar.DAY_OF_YEAR) < joinedCalenderDate.get(Calendar.DAY_OF_YEAR)) {
            years--;
        }

        if (years < 2) {
            if(employee.getRole().equals(Role.MANAGER)) return 18000.0;
            return 10000.0;
        } else {
            if(employee.getRole().equals(Role.MANAGER)) return 15000.0;
            return 120000.0;
        }
