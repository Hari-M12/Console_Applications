...
class WashRecordStatusUpdateModel {
    ...
    private final WashRecordStatusUpdateView washRecordStatusUpdateView;

    WashRecordStatusUpdateModel(WashRecordStatusUpdateView washRecordStatusUpdateView){
        this.washRecordStatusUpdateView = washRecordStatusUpdateView;
    }

    void updateWashRecordStatus(Long id,WashStatus oldWashStatus,WashStatus selectedWashStatus) {
        WashRecord fetched = WashTrackDB.getInstance().getWashRecordById(id);

        fetched.setWashStatus(selectedWashStatus);
        WashRecord updated = WashTrackDB.getInstance().updateWashRecordStatus(fetched);
        if(updated == null) {
            washRecordStatusUpdateView.showErrorMessage("Updation failed");
        }else {
            washRecordStatusUpdateView.onUpdateSuccess(oldWashStatus,selectedWashStatus);
        }
    ...
    void createWashRecord(Vehicle vehicle, WashType washType, WashStatus washStatus, Payment payment, Long washDate, String customerPhone, Long assignedTo) {
        ...
        if(saved == null){
            washRecordStatusUpdateView.showErrorMessage("error occured during saving washRecord");
            return;
        }else {
            washRecordStatusUpdateView.onSuccess("Successfully stored washRecord");
        }
    ...
    List<WashRecord> getPendingWashRecords(Employee employee) {

        if(employee.getRole().equals(Role.MANAGER) || employee.getRole().equals(Role.OWNER)){
            return WashTrackDB.getInstance().getAllPendingWashRecord();
        }else if(employee.getRole().equals(Role.EMPLOYEE)){
            Long id  = employee.getId();
            if(id == null) return Collections.emptyList();
            return WashTrackDB.getInstance().getPendingWashRecordsOf(id);
        }else{
    ...
    WashRecord getWashRecordWithId(Long id) {
        ...
        WashRecord fetched = WashTrackDB.getInstance().getWashRecordById(id);
        if(fetched == null){
            washRecordStatusUpdateView.showErrorMessage("WashRecord Not Found");
            return null;
        }
        return fetched;
    }
